...
{
  "trace_enabled": true,
  "session_checkpoint_enabled": true,
  "session_resume_on_start": false,
  "tool_permission_mode": "auto"
}
```

运行态文件统一写入工作区根目录 `.agent/`，包括日志、trace、session checkpoint、todo 和任务状态；该目录不应提交到仓库。

模型引用统一使用 `provider:model` 格式，例如：

```bash
python -m agent --provider openai --model gpt-4o
python -m agent --provider claude --model claude-sonnet-4-20250514
python -m agent --provider ollama --model qwen2.5-coder:7b
```
...
```bash
python -m agent -p "请写一个两数之和的 Python 函数"      
```

### 管道 / 重定向（Raw 输出）

```bash
python -m agent -p "生成一个 Flask hello world" > app.py
cat src/api.py | python -m agent -p "Review 这段代码，找出类型不安全的地方"
```
...
- `agent/__main__.py`：统一入口（兼容 `python -m agent` 与 `python agent`）
- `agent/agent.py`：核心代理循环
- `agent/ui/tui.py`：TUI 交互界面
- `agent/ui/cli.py`：CLI 交互界面
- `agent/hooks.py`：Hook 系统
- `agent/skills.py`：Skill 加载与注入
- `agent/tools/`：工具定义与注册
